...
class CondInst(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.boxverd_enabled:
            ...
            # sem_loss is off so no loss is actually calculated and the dict will be empty
            mask_feats, sem_losses = self.mask_branch(features_unshuffled, gt_instances=None)
            mask_feats_shuffled, _ = self.mask_branch(features_shuffled, gt_instances=None)
            # breakpoint()
        ...
        if self.training:
            ...
            # if self.boxverd_enabled: to visualize seg0
            #     pred_instances_w_masks = self._forward_mask_heads_test(
            #         proposals, mask_feats_shuffled
            #     )
            #     if len(pred_instances_w_masks) > 0 and pred_instances_w_masks.has("pred_global_masks"):
            #         # pred_global_masks are at mask_out_stride resolution; sigmoid is
            #         # pointwise, so deshuffling the scores equals deshuffling logits.
            #         mask_shuffler = shuffler.scale(
            #             (1.0 / self.mask_out_stride, 1.0 / self.mask_out_stride)
            #         )
            #         pred_instances_w_masks.pred_global_masks = mask_shuffler.unshuffle(
            #             pred_instances_w_masks.pred_global_masks
            #         )
    ...
    def _forward_mask_heads_train(self, proposals, mask_feats, gt_instances, mask_feats_shuffled=None, shuffler=None):
        ...
        pred_instances.mask_head_params = pred_instances.top_feats # 704, 233 
        # Each proposal carries the complete weights+biases of its personal 3-layer mask decoder, generated by the controller.
        # breakpoint()
